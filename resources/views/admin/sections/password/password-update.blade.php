...
@section('panel-header')
    <div>
        <h1 class="panel-title">Edit Password</h1>
        <ul class="breadcrumb">
            <li><a href="{{ route('admin.view.dashboard') }}">Admin</a></li>
            <li><i data-feather="chevron-right"></i></li>
            <li><a href="{{ route('admin.view.password.list') }}">Password Manager</a></li>
            <li><i data-feather="chevron-right"></i></li>
            <li><a href="{{ route('admin.view.password.update',['id' => $password->id]) }}">Edit Password</a></li>
        </ul>
    </div>
@endsection

@section('panel-body')
    <form action="{{ route('admin.handle.password.update',['id' => $password->id]) }}" method="POST">
        @csrf
            ...
            <div class="panel-card-body">
                <div class="grid md:grid-cols-4 sm:grid-cols-1 md:gap-7 sm:gap-5">
                    ...
                    <div class="flex flex-col">
                        ...
                        <select class="input-box-md" name="customer_id">
                            ...
                            @foreach ($customers as $customer)
                                <option @selected($password->customer_id == $customer->id) value="{{ $customer->id }}">{{ $customer->name }} ({{$customer->company_name}})</option>
                            @endforeach
                        </select>
                        @error('customer_id')
                    ...
                    <div class="flex flex-col">
                        ...
                        <select class="input-box-md" name="type" required>
                            ...
                            @foreach ($password_type as $type)
                                <option @selected($password->type == $type) value="{{ $type }}">{{ $type }}</option>
                            @endforeach
                        </select>
                        @error('type')
                            <span class="input-error">{{ $message }}</span>
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="text" name="username" value="{{ old('username',$password->username) }}"
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="email" name="email" value="{{ old('email',$password->email) }}"
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="tel" name="phone" value="{{ old('phone',$password->phone) }}"
                    ...
                    <div class="flex flex-col">
                        ...
                        <input type="text" name="password" value="{{ old('password',$password->password) }}"
